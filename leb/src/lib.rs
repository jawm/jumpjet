...
pub fn signed<R: Read>(buffer: & R) -> Result<i64, Error> {
...
pub fn unsigned<R: Read>(buffer: & R) -> Result<u64, Error> {
	let mut result: u64 = 0;
	let mut shift = 0;
	for object in buffer.bytes() {
...
mod tests {
	...
	fn x(bytes: Vec<u8>) -> Cursor<Vec<u8>> {
    ...
    fn unsigned_reads_1_32() {
    	let buff = x(vec![1]);
    	assert!(unsigned(&buff).unwrap() == 1);
    }

    #[test]
    fn unsigned_reads_2_32() {
    	let mut buff = x(vec![2]);
    	assert!(unsigned(&buff).unwrap() == 2);
    }
    ...
    fn unsigned_reads_500_32() {
    	...
    	assert!(unsigned(&buff).unwrap() == 500);
    }

    #[test]
    fn signed_reads_1() {
    	let mut buff = x(vec![1]);
    	assert!(signed(&buff).unwrap() == 1);
    }

    #[test]
    fn signed_not_reads_2() {
    	let mut buff = x(vec![1]);
    	assert!(signed(&buff).unwrap() != 2);	
    }
}
